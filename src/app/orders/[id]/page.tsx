...
import { OrderSupportSection } from "./order-support/order-support-section";
...
export default async function OrderDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ placed?: string }> }) {
  ...
  try {
    ...
    const supportRequests = await OrderSupportService.getForOrder(user.id, id);
